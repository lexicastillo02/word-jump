...
@onready var log_off_button: Button = $StartMenu/LogOffButton

var is_icon_selected: bool = false
var is_start_menu_open: bool = false
...
func _input(event: InputEvent) -> void:
	...
	# Click elsewhere to deselect icon and close menu
	if event is InputEventMouseButton and event.pressed:
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			# Close start menu if clicking outside of it and not on start button
			if is_start_menu_open:
				var menu_rect := start_menu.get_global_rect()
